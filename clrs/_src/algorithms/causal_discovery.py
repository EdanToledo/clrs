...
def IC_star(X_df: _DataFrame) -> _Out:
  ...
  VAR_NAMES = list(X_df.columns.sort_values()) # NOTE: this is sorted for consistency in adjacency matrix and arrows_mat representation
  ...
  for N in range(max_k + 1): 
      for (a, b) in list(g.edges()): # iterate over all pairs of nodes
          ...
          for S_ab in itertools.combinations(c_candidates, N): # iterate over subsets of size N
              ...
              if test.independent():
                  ...
                  probing.push(
                    ...
                    next_probe={
                      ...
                      'node_3': np.zeros(NUM_VARS),
                      'S_12': probing.mask_set([VAR_NAMES.index(node) for node in list(S_ab)] , NUM_VARS),
                      'A_h': np.copy(nx.to_numpy_array(g)),
                      'arrows_h': np.copy(arrows_mat)
                    }
                  )
              ...
              if S_ab != None and c not in S_ab:
                  ...
                  probing.push(
                    ...
                    next_probe={
                      ...
                      'node_3': probing.mask_one(b_idx, NUM_VARS), # TODO: check if this makes sense
                      'S_12': probing.mask_set([VAR_NAMES.index(node) for node in list(S_ab)], NUM_VARS),
                      'A_h': np.copy(nx.to_numpy_array(g)),
                      'arrows_h': np.copy(arrows_mat)
                    }
  ...
  added_arrows = True
  while added_arrows:
      R1_added_arrows = apply_recursion_rule_1(g, probes, arrows_mat, VAR_NAMES, NUM_VARS)
      R2_added_arrows = apply_recursion_rule_2(g, probes, arrows_mat, VAR_NAMES, NUM_VARS)
...
def apply_recursion_rule_1(g, probes, arrows_mat, VAR_NAMES, NUM_VARS):
  ...
  for c in g.nodes():
      for (a,b) in itertools.combinations(g.neighbors(c), 2):
          ...
          if not g.has_edge(a,b):
            if (arrows_mat[a_idx, c_idx] >= 1) and (arrows_mat[b_idx, c_idx] == 0) and not (arrows_mat[c_idx, b_idx] == 2):
                ...
                probing.push(
                    ...
                    next_probe={
                      ...
                      'node_2': probing.mask_one(VAR_NAMES.index(b), NUM_VARS),
                      'node_3': probing.mask_one(VAR_NAMES.index(a), NUM_VARS), # TODO: check if this is the right ordering
                      'S_12': np.zeros(NUM_VARS),
                      'A_h': np.copy(nx.to_numpy_array(g)),
                      'arrows_h': np.copy(arrows_mat)
                    }
                  )
            if (arrows_mat[b_idx, c_idx] >= 1) and (arrows_mat[a_idx, c_idx] == 0) and not (arrows_mat[c_idx, a_idx] == 2):
                ...
                probing.push(
                    ...
                    next_probe={
                      ...
                      'node_3': probing.mask_one(VAR_NAMES.index(b), NUM_VARS), # TODO: check if this is the right ordering
                      'S_12': np.zeros(NUM_VARS),
                      'A_h': np.copy(nx.to_numpy_array(g)),
                      'arrows_h': np.copy(arrows_mat)
                    }
...
if __name__ == "__main__":
  ...
  graph_comp_2 = IraC(RobustRegressionTest).search(X_df_2, variable_types_2)
  for i, j in graph_comp_2.edges():
      print(i,j, graph_comp_2.get_edge_data(i, j ))

  pos = nx.planar_layout(graph_comp_2)
