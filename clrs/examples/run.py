...
flags.DEFINE_integer(
    "nb_msg_passing_steps", 1, "Number of message passing steps to run per hint."
)
flags.DEFINE_float("learning_rate", 0.001, "Learning rate to use.")
flags.DEFINE_float(
    "grad_clip_max_norm", 1.0, "Gradient clipping by norm. 0.0 disables grad clipping"
)
...
def _maybe_download_dataset(dataset_path):
    ...
    dataset_folder = (
        dataset_path  # <- TODO: check if need to change this back to the above
    )
...
def main(unused_argv):
    ...
    (
        ...
        test_sample_counts,
        spec_list,
    ) = create_samplers(rng, train_lengths)
    ...
    eval_model = clrs.models.BaselineModel(
        spec=spec_list, dummy_trajectory=[next(t) for t in val_samplers], **model_params
    )
    if FLAGS.chunked_training:
            ...
            wandb.log(
                {
                    "train_step": step,
                    "train_loss": cur_loss,
                    "train_items": current_train_items[algo_idx],
                }
            )
